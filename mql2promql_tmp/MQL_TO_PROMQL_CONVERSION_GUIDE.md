...
| Availability | VM Availability | MQL | `fetch k8s_container \| metric 'kubernetes.io/anthos/kubevirt_vmi_network_transmit_bytes_total' \| ... \| align rate(2m) \| ...` | The MQL query checks for network activity to determine VM availability. The PromQL query calculates the rate of transmitted bytes and checks if it's greater than 0. | PromQL | `sum by (kubernetes_vmi_label_kubevirt_vm) (rate(kubernetes_io:anthos_kubevirt_vmi_network_transmit_bytes_total{monitored_resource="k8s_container",${cluster_name},cluster_name=~"${market.value}.*"}[2m])) > 0` | Similar to node availability, this is an indirect way of checking for VM status. |
| Peformance | Node CPU Utilization | MQL | `fetch k8s_node \| metric 'kubernetes.io/anthos/node/cpu/allocatable_utilization' \| ... \| group_by 1m, [mean(...)] \| scale '%'` | The MQL query calculates the mean of a gauge metric over a 1-minute window. This is translated to `avg_over_time` in PromQL. The `scale '%' ` is converted to multiplication by 100. | PromQL | `avg_over_time(kubernetes_io:anthos_node_cpu_allocatable_utilization{monitored_resource="k8s_node",${cluster_name},cluster_name=~"${market.value}.*"}[1m]) * 100` | |
