...
| `dashboards/gdc-vm-distribution.json`                      | Yes           | Uses `timeSeriesQueryLanguage`.                                       | WIP               |
| `dashboards/gdc-vm-view.json`                              | Yes (Partial) | Mixed MQL (`timeSeriesQueryLanguage`, `timeSeriesFilter`) and PromQL. | WIP               |
...
| Group | Input Query Title | Input Query Type | Source Query | LLM Reasoning | LLM Output Query Type | LLM Output PromQL Query | Comments |
| --- | --- | --- | --- | --- | --- | --- | --- |
| N/A | Node 1 | MQL | `{ t_0 : fetch prometheus_target \| ... \| filter metric.pod =~ '(virt-launcher).*' \| ...; t_1: fetch prometheus_target \| ... \| filter metric.pod =~ '(virt-launcher).*' \| ... } \| join \| ...` | The MQL query joins pod status with pod info to get running virt-launcher pods on a specific node. The PromQL equivalent uses a multiplication (`*`) between `kube_pod_status_phase` and `kube_pod_info` with appropriate labels. | PromQL | `(kube_pod_status_phase{phase="Running", pod=~"(virt-launcher).*", cluster_name="${cluster_name.value}"} * on(pod) group_left(node) kube_pod_info{pod=~"(virt-launcher).*", node=~".*01.ba.l.google.com$$", cluster_name="${cluster_name.value}"}) > 0` | |
| N/A | Node 2 | MQL | `{ t_0 : fetch prometheus_target \| ... \| filter metric.pod =~ '(virt-launcher).*' \| ...; t_1: fetch prometheus_target \| ... \| filter metric.pod =~ '(virt-launcher).*' \| ... } \| join \| ...` | Similar to the Node 1 query, but for a different node. | PromQL | `(kube_pod_status_phase{phase="Running", pod=~"(virt-launcher).*", cluster_name="${cluster_name.value}"} * on(pod) group_left(node) kube_pod_info{pod=~"(virt-launcher).*", node=~".*02.ba.l.google.com$$", cluster_name="${cluster_name.value}"}) > 0` | |
| N/A | Node 3 | MQL | `{ t_0 : fetch prometheus_target \| ... \| filter metric.pod =~ '(virt-launcher).*' \| ...; t_1: fetch prometheus_target \| ... \| filter metric.pod =~ '(virt-launcher).*' \| ... } \| join \| ...` | Similar to the Node 1 query, but for a different node. | PromQL | `(kube_pod_status_phase{phase="Running", pod=~"(virt-launcher).*", cluster_name="${cluster_name.value}"} * on(pod) group_left(node) kube_pod_info{pod=~"(virt-launcher).*", node=~".*03.ba.l.google.com$$", cluster_name="${cluster_name.value}"}) > 0` | |
| N/A | Node 1 | MQL | `{ t_0 : fetch prometheus_target \| ... \| filter metric.pod =~ '(robin-master).*' \| ...; t_1: fetch prometheus_target \| ... \| filter metric.pod =~ '(robin-master).*' \| ... } \| join \| ...` | The MQL query joins pod status with pod info to get running robin-master pods on a specific node. The PromQL equivalent uses a multiplication (`*`) between `kube_pod_status_phase` and `kube_pod_info` with appropriate labels. | PromQL | `(kube_pod_status_phase{phase="Running", pod=~"(robin-master).*", cluster_name="${cluster_name.value}"} * on(pod) group_left(node) kube_pod_info{pod=~"(robin-master).*", node=~".*01.ba.l.google.com$$", cluster_name="${cluster_name.value}"}) > 0` | |
| N/A | Node 2 | MQL | `{ t_0 : fetch prometheus_target \| ... \| filter metric.pod =~ '(robin-master).*' \| ...; t_1: fetch prometheus_target \| ... \| filter metric.pod =~ '(robin-master).*' \| ... } \| join \| ...` | Similar to the Node 1 (Robin Master) query, but for a different node. | PromQL | `(kube_pod_status_phase{phase="Running", pod=~"(robin-master).*", cluster_name="${cluster_name.value}"} * on(pod) group_left(node) kube_pod_info{pod=~"(robin-master).*", node=~".*02.ba.l.google.com$$", cluster_name="${cluster_name.value}"}) > 0` | |
| N/A | Node 3 | MQL | `{ t_0 : fetch prometheus_target \| ... \| filter metric.pod =~ '(robin-master).*' \| ...; t_1: fetch prometheus_target \| ... \| filter metric.pod =~ '(robin-master).*' \| ... } \| join \| ...` | Similar to the Node 1 (Robin Master) query, but for a different node. | PromQL | `(kube_pod_status_phase{phase="Running", pod=~"(robin-master).*", cluster_name="${cluster_name.value}"} * on(pod) group_left(node) kube_pod_info{pod=~"(robin-master).*", node=~".*03.ba.l.google.com$$", cluster_name="${cluster_name.value}"}) > 0` | |
