...
| --- | --- | --- | --- | --- | --- | --- | --- |
| N/A | Config Sync is down for more than 30 minutes | MQL | `fetch prometheus_target \| metric 'prometheus.googleapis.com/config_sync_status/gauge' \| ...` | The final query uses `count_over_time` for better data visibility in the GCM UI and aggregates by cluster for more meaningful alerts, which is an improvement over the original MQL. | PromQL | `sum by (cluster_name) (count_over_time(external_googleapis_com:prometheus_config_sync_resource_count{monitored_resource="k8s_container"}[30m])) == 0` | The new alert is more granular, firing if all ConfigSync metrics within a single cluster disappear. |
