# MQL to PromQL Conversion Guide - CoreDNS SERVFAIL Ratio

This document explains the proposed translations to Prometheus Query Language (PromQL) of the `alerts/system/coredns-servfail-ratio-1-percent.yaml` alert.

## `alerts/system/coredns-servfail-ratio-1-percent.yaml`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| N/A | CoreDNS SERVFAIL count ratio: SERVFAIL counts / all response counts | MQL | `fetch k8s_container \| metric 'kubernetes.io/anthos/coredns_dns_responses_total' \| ...` | The MQL `align delta` is converted to PromQL `increase()`. The ratio is calculated using division. Grouping is by `cluster_name`, `location`, `project_id`. The `join` for baremetal clusters is done with `and on()`. | PromQL | `((sum by (cluster_name, location, project_id) (increase(kubernetes_io:anthos_coredns_dns_responses_total{rcode="SERVFAIL"}[5m]))) / (sum by (cluster_name, location, project_id) (increase(kubernetes_io:anthos_coredns_dns_responses_total[5m])))) and on(cluster_name, location, project_id) (kubernetes_io:anthos_anthos_cluster_info{anthos_distribution="baremetal", monitored_resource="k8s_container"}) > 0.01` | The corrected PromQL query is functionally equivalent to the original MQL alert, successfully returning data and displaying consistent trends in the GCM UI. |
