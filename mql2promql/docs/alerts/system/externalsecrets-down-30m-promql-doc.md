# MQL to PromQL Conversion Guide - External Secrets Down

This document explains the proposed translations to Prometheus Query Language (PromQL) of the `alerts/system/externalsecrets-down-30m.yaml` alert.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| N/A | External Secrets is down | MQL | `fetch k8s_container \| metric 'kubernetes.io/anthos/container/uptime' \| ...` | The MQL `absent_for` after a `join` is translated to a PromQL `unless on(...)` operator to achieve per-cluster absence detection. | PromQL | `(max by (project_id, location, cluster_name) (kubernetes_io:anthos_anthos_cluster_info{anthos_distribution="baremetal", monitored_resource="k8s_container"})) unless on(project_id, location, cluster_name) (avg by (project_id, location, cluster_name) (kubernetes_io:anthos_container_uptime{container_name=~"external-secrets", monitored_resource="k8s_container"}))` | The converted PromQL alert is a correct and reliable replacement for the original MQL policy. |
