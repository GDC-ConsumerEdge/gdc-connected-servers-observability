# MQL to PromQL Conversion Guide - CoreDNS Down

This document explains the proposed translations to Prometheus Query Language (PromQL) of the `alerts/system/coredns-down.yaml` alert.

## `alerts/system/coredns-down.yaml`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| N/A | CoreDNS is up | MQL | `fetch k8s_container \| metric 'kubernetes.io/anthos/container/uptime' \| ...` | The MQL `absent_for` after a `group_by` is translated to a PromQL `unless` operator to achieve per-cluster absence detection. `label_replace` is used for label alignment. | PromQL | `(max by (cluster, location, project_id) (label_replace(kubernetes_io:anthos_anthos_cluster_info{anthos_distribution="baremetal", monitored_resource="k8s_container"}, "cluster", "$1", "cluster_name", "(.*)"))) unless (avg by (cluster, location, project_id) (kubernetes_io:anthos_container_uptime{container_name=~"coredns", monitored_resource="k8s_container"}))` | The PromQL alert successfully generates a distinct time series for each affected cluster, mirroring the MQL's per-cluster alerting behavior. |
