# MQL to PromQL Conversion Guide - Node Memory Usage High

This document explains the proposed translations to Prometheus Query Language (PromQL) of the `alerts/node/node-memory-usage-high.yaml` alert.

## `alerts/node/node-memory-usage-high.yaml`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| N/A | Node allocatable memory bytes percent | MQL | `fetch prometheus_target \| metric 'kubernetes.io/anthos/kube_node_status_allocatable/gauge' \| ...` | The original MQL metrics were unavailable. The query was adapted to use reliable metrics for allocatable and capacity memory, mirroring the successful CPU usage alert. It calculates the ratio and joins with cluster info to filter for baremetal clusters. | PromQL | `(sum by(node, cluster, location, project_id) (kubernetes_io:anthos_kube_node_status_allocatable_gauge{resource="memory"}) / sum by(node, cluster, location, project_id) (kubernetes_io:anthos_kube_node_status_capacity_gauge{resource="memory"})) and on(cluster, location, project_id) (max by(cluster, location, project_id) (label_replace(kubernetes_io:anthos_anthos_cluster_info{anthos_distribution="baremetal", monitored_resource="k8s_container"}, "cluster", "$1", "cluster_name", "(.*)"))) < 0.2` | The final query is more specific than the original MQL by adding a filter for baremetal clusters. |
