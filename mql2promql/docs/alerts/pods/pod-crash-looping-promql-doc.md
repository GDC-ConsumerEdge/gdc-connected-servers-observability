# MQL to PromQL Conversion Guide - Pod Crash Looping

This document explains the proposed translations to Prometheus Query Language (PromQL) of the `alerts/pods/pod-crash-looping.yaml` alert.

## `alerts/pods/pod-crash-looping.yaml`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| N/A | Pod restart times [15m] | MQL | `fetch prometheus_target \| metric 'kubernetes.io/anthos/kube_pod_container_status_restarts_total/counter' \| ...` | The MQL `align delta` is converted to PromQL `increase()`. The `join` to filter for baremetal clusters is converted to a `* on(cluster) group_left()` operation with `label_replace` to align cluster labels. | PromQL | `increase(kubernetes_io:anthos_kube_pod_container_status_restarts_total_counter{pod!~"^(bm-system|robin-prejob).*"}[15m]) * on(cluster) group_left() (label_replace(max(kubernetes_io:anthos_anthos_cluster_info{anthos_distribution="baremetal", monitored_resource="k8s_container"}) by (cluster_name), "cluster", "$1", "cluster_name", "(.*)")) > 0` | **Critical Issue:** The converted PromQL alert does not fire, even though the query is logically correct and returns data in Metrics Explorer. This is a major functional regression. |
