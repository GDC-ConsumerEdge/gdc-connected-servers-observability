...
for alert in $(gcloud alpha monitoring policies list --uri --filter='user_labels.managed:mql2promql'); do
  ...
  for file in "$dir"/*; do
    if [ -f "$file" ]; then
      gcloud alpha monitoring policies create \
        ...
        --user-labels=managed=mql2promql # --notification-channels=$NOTIFICATION_CHANNEL_ID
    fi
  done
done